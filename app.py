...
def send_to_gemini(prompt):
    ...
    csv_output_path = os.path.join(UPLOAD_FOLDER, 'output.csv')
    with open(csv_output_path, 'w', encoding='utf-8') as textfile:
...
def upload():
    ...
    if file and text_input:
        ...
        csv_output_path = send_to_gemini(prompt)

        # Convert the output CSV to Excel
        excel_output_path = csv_to_excel(csv_output_path, file.filename)

        # Render the download button for the processed Excel file
        return render_template('index.html', download_link=excel_output_path)
